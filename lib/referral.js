...
export async function ensureUserReferralData(uid, email = "", candidateReferrerCode = "", authToken = null) {
  ...
  return withReferralLock(async () => {
    ...
    // 1. Assign permanent unique referral code if missing
    if (!referralCode) {
      referralCode = generateReferralCode();
      updates.referralCode = referralCode;
      updates.referralLink = `https://solvatech.name.ng/${referralCode}`;
      needsUpdate = true;
    } else if (!merged.referralLink) {
      updates.referralLink = `https://solvatech.name.ng/${referralCode}`;
      needsUpdate = true;
    }
    ...
    const finalRecord = {
      ...
      referralLink: updates.referralLink || merged.referralLink || `https://solvatech.name.ng/${referralCode}`,
      referredBy,
...
export async function getReferralStats(uid) {
  ...
  const referralLink = userData.referralLink || `https://solvatech.name.ng/${referralCode}`;
