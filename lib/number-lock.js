...
/**
 * Super Admin Action: Wipes current number lock from account.
 * Preserves full historical audit record marked as REMOVED BY ADMIN.
 */
export async function wipeNumberFromAccount(uid, adminEmail = "", reason = "Removed by administrator") {
  ...
  return withAtomicLock(async () => {
    ...
    if (phone && inMemoryLocks.locks[phone]) {
      userEmail = inMemoryLocks.locks[phone].userEmail || "";
    }

    const lockedAt = inMemoryLocks.locks[phone]?.lockedAt || null;
    ...
    // Add historical record with REMOVED BY ADMIN status
    ...
    const histRecord = {
      ...
      timestamp: nowIso,
      adminEmail,
      note: reason || "Wiped by Super Admin",
    };
    ...
    await persistLocalStore();
    logger.info(`Super Admin ${adminEmail} wiped WhatsApp number ${phone || "N/A"} from user ${uid}`);
