...
export async function handleDeletedMessage(userId, payload, sock, botSentMessageIds = null) {
  ...
  // Send automatic . reply associated with that specific deleted message
  ...
  try {
    ...
    if (original?.rawMessage) {
      try {
        ...
        logger.debug("Quoted reply failed for . alert, sending unquoted fallback", err.message);
        alertMsg = await sock.sendMessage(remoteJid, {
          text: ".",
        });
      }
    ...
    if (alertMsg?.key?.id) {
      ...
      if (botSentMessageIds && typeof botSentMessageIds.add === "function") {
        botSentMessageIds.add(alertMsg.key.id);
      }
    }
  } catch (error) {
    logger.warn("Could not send automatic . alert notification", error.message);
  }
