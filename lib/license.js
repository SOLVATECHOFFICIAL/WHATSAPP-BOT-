...
export async function listAllLicenses() {
  ...
  // Also try pulling latest from Firestore if possible
  const db = getFirebaseServerFirestore();
  if (db) {
    try {
      ...
      snap.forEach((docSnap) => {
        ...
        if (data && data.code) {
          inMemoryLicenses[data.code] = { ...inMemoryLicenses[data.code], ...data };
        }
      });
    } catch (err) {
 ...
 */
export async function getUserLicenseStatus(uid) {
  if (!uid) return { hasActiveLicense: false, status: "none" };
