...
export function createWhatsAppController(options = {}) {
  ...
  async function processMessage(message) {
    ...
    if (message.message?.protocolMessage?.type === 0 || message.message?.protocolMessage?.type === "REVOKE") {
      await handleDeletedMessage(userId, message.message, sock, botSentMessageIds);
      return;
    }
    ...
    sock.ev.on("messages.update", (updates) => {
      for (const update of updates) {
        if (update.update?.protocolMessage?.type === 0 || update.update?.protocolMessage?.type === "REVOKE") {
          messageQueue.add(update.key?.remoteJid, () => handleDeletedMessage(userId, update.update, sock, botSentMessageIds)).catch((error) => {
