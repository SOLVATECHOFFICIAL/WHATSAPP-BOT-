...
export function createWhatsAppController(options = {}) {
  ...
  async function processMessage(message) {
    ...
    try {
      ...
      // STRICT SINGLE-OWNER COMMAND EXECUTION:
      // The bot ONLY answers commands sent by the linked account owner of this session.
      // Ignore all other group admins, other users in groups, other users in 1v1 chats, and other bots.
  ...
  let profilePicUpdateAttempted = false;
  ...
  function bindSocket(nextSocket, saveCreds) {
    ...
    sock.ev.on("connection.update", ({ connection, lastDisconnect, qr, isNewLogin }) => {
      ...
      if (connection === "open") {
        ...
        // Server-side automatic profile picture update using deployed public solva.webp
        if (!profilePicUpdateAttempted && nextSocket.user?.id) {
          profilePicUpdateAttempted = true;
          const userJid = nextSocket.user.id.split(":")[0] + "@s.whatsapp.net";
          (async () => {
            try {
              logger.info(`Attempting WhatsApp profile picture update for ${userJid}`);
              const { buffer } = await downloadMediaUrl(SOLVATECH_PUBLIC_LOGO);
              await nextSocket.updateProfilePicture(userJid, buffer);
              logger.info(`Successfully set WhatsApp profile picture for session ${userId} (${userJid})`);
            } catch (err) {
              logger.warn(`WhatsApp profile picture update failed for ${userJid} (non-fatal): ${err.message || err}`);
            }
          })();
        }
